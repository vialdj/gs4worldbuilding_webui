...
from gs4worldbuilding import units, model, terrestrial, asteroid_belt
...
def is_planet(obj):
    return issubclass(type(obj), planet.Planet)

@app.template_test('terrestrial')
def is_terrestrial(obj):
    return issubclass(type(obj), terrestrial.Terrestrial)

@app.template_test('satellite')
def is_satellite(obj):
    return issubclass(type(obj._orbit._parent_body), planet.Planet)

@app.template_test('asteroid_belt')
def is_asteroid_belt(obj):
    return issubclass(type(obj), asteroid_belt.AsteroidBelt)
...
def diameterFormat(value):
    s = '{:.2f} km'.format(value.to(u.km).value)
    if value.to(units.D_jup).value > 5:
        return '{} ({:.2f} D☉)'.format(s, value.to(units.D_sun).value)
    elif value.to(units.D_earth).value > 5:
        return '{} ({:.2f} D♃)'.format(s, value.to(units.D_jup).value)
    return '{} ({:.2f} D🜨)'.format(s, value.to(units.D_earth).value)
...
def scaleRadius(value):
    scale = (biggest_world.diameter / 2 if
             ...
             biggest_star.radius)
    scale = np.sqrt(scale.to(units.D_earth).value)
    return int(round(np.sqrt(value.to(units.D_earth).value) / scale * 50))
...
def scaleDiameter(value):
    scale = (biggest_world.diameter / 2 if
             ...
             biggest_star.radius)
    scale = np.sqrt(scale.to(units.D_earth).value)
    return int(round(np.sqrt(value.to(units.D_earth).value / 2) / scale * 50))
...
def atmoPropertiesFormat(value):
    ...
    if issubclass(type(value), terrestrial.Marginal):
        properties.append(' '.join(re.findall('[A-Z][^A-Z]*', value.__class__.__name__)))
    if value.suffocating:
        properties.append('Suffocating')
    if value.toxicity:
        d = {terrestrial.Atmosphere.Toxicity.NONE: 'Negligible',
             terrestrial.Atmosphere.Toxicity.MILD: 'Mild',
             terrestrial.Atmosphere.Toxicity.HIGH: 'High',
             terrestrial.Atmosphere.Toxicity.LETHAL: 'Lethal'}
        if type(value.toxicity) == model.bounds.ValueBounds:
...
def random_star_system():
    star_system = Builder.build_star_system()
